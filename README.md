...
公告（9-10）：内容包顶层可以写 `"notice": { "id": "terms-2026-10", "zh": "…", "en": "…", "ja": "…", "until": "2026-10-31", "url": "https://…" }`。App 开机或拉到新包时弹一次，看过的 id 不再弹，换 id 才会再弹；新包不写 notice 就等于撤掉。服务协议里承诺的两种「App 内公告」都走这里：条款实质变更（至少提前 7 日）、停止运营（至少提前 30 日）。只有 id + zh 是必填。

协议同意（9-10）：首次打开 / 老用户升级后弹一次同意窗，同意的版本记在 settings.legalOk。服务协议或隐私政策**实质更新**时，改 `app/js/legal.js` 的 `LEGAL_VER`（跟协议页首的生效日期一致）并发版，所有人会再弹一次。不同意照样能盖章，登录、购买、分享时会再问。
